...
TokenStack* token_stack_create()
{
    ...
    result_token_stack->size = DEFAULT_TOKEN_STACK_SIZE;
    result_token_stack->top  = 0;

    return result_token_stack;
}

void resize_array(TokenStack* tokstk, size_t new_size)
{
    Token* tmp = realloc(tokstk->arr, sizeof(Token) * new_size);
    if (tmp != NULL) 
    {
        tokstk->arr  = tmp;
        tokstk->size = new_size;
    }
}

void token_stack_push(TokenStack *tokstk, Token tok)
{
    if (tokstk->top >= tokstk->size)
        resize_array(tokstk, tokstk->size * 2);
